...
running session, sibling of [`sbx proc`](proc.md) (processes) and [`sbx net`](net.md) (egress).
...
([`sbx run --observe`](run.md#observing-a-run-observe)).

See also: [`sbx proc`](proc.md) · [`sbx net`](net.md) · [`sbx session`](session.md).
...
[`sbx run --observe`](run.md#observing-a-run-observe) or
[`sbx app run <name> --observe`](app.md) — the same `--observe` that feeds
[`sbx proc logs`](proc.md#logs). A session without it is reported as *unobserved*, not shown empty.
...
[`sbx proc logs`](proc.md#logs), the **only** way to watch a [detached](run.md) (`--detach`) one.
...
- **build/VCS/vendor trees** — `.git`, `node_modules`, `target`, `.venv` — the way
  [`sbx proc`](proc.md) filters `bwrap`/`systemd-run` plumbing (a single `git commit` writes hundreds
