...
| `--config` / `--env` / `--net` / `--gui` / `--proc` / `--notify` / `--nixpkgs` / `--bind` / `--forward` / `--limit` / `--package` / `--seccomp` / `--device` / `--gpu` / `--audio` / `--dbus` | typed one-shot [overrides](../configuration/overrides.md), applied **after** the app's overlay (the final word) |
