...
| `use` | security | tool [bundles](bundles.md) folded into this app, in the order written (a later one wins on a key, the app always wins); must sit above the first `[table]` header |
...
| `flakes`, `tarball`, `deb`, `appimage` | security | the resolver tables pairing with this app's packages, `[app.<name>.flakes.<tool>]` etc. (see [packages](packages.md)) |
| `network` | security | overrides the baseline posture when set |
| `proc` | security | overrides the baseline exec posture when set (see [proc](proc.md)) |
| `notify` | security | overrides the baseline refusal-notification policy when set (see [notify](notify.md)) |
| `gui` | security | overrides the baseline when set |
| `gpu`, `audio`, `dbus` | security | override the baseline when set (see [gpu](gpu.md), [audio](audio.md), [dbus](dbus.md)) |
| `forward` | security | host loopback ports **unioned** onto the baseline's (see [forward](../networking/forward.md)) |
| `secret` | security | credentials for this app's egress |
| `task` | security | this app's declared operations, `[app.<name>.task.<task>]`, unioned onto the baseline's (see [task](task.md)) |
| `limits` | security | per-field override of the baseline cgroup limits |
| `seccomp`, `devices`, `ssh_agent` | security | unioned onto the baseline's — which is how a deploy key is granted to *one* agent rather than every cage (see [seccomp](seccomp.md), [devices](devices.md), [ssh-agent](ssh-agent.md)) |
