...
| `[exec.<program>]` | what one of those programs may run in turn (see [below](#what-each-program-may-run-in-turn-exec)) |
...
What `git` may then run is [a section of its own](#what-each-program-may-run-in-turn-exec), naming it here would let the command run it directly instead.
...
hundred names, as [above](#what-each-program-may-run-in-turn-exec). That caller is the
