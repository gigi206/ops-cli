...
run with a credential the caller never holds, see [`[task.<name>]`](task.md)) carries
...
`bundle:<name>` in the `DECLARED IN` column of [`sbx task list`](../cli/task.md) (and
in [`sbx task show`](../cli/task.md)) — the fold makes the entry look like the app's
