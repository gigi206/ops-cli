...
`forward` declares a host loopback TCP port ops forwards *into* the cage, so a host process
...
- **Loopback only.** ops never binds an external interface; the port is reachable only from
...
- **Trusted-only.** `forward` is a security field, gated exactly like `network`/`gui`:
  honored from the global `ops.toml` (trusted by location), a trusted project, or a named
...
- **Under `network = "none"` or `"deny"`/`"allow"`/`"ask"`** (empty netns) the forwarder
  wires: ops binds the host port, a per-launch dir is bound into the cage, and an in-cage
  `socat` bridges the Unix socket to the cage loopback. The browser connects to the host
  port; ops pumps each connection through the socket into the cage.
- **Under `network = "shared"`** the cage shares the host netns, so a cage service on
  ...
  ops skips it with a note.
- **Collision = fail-closed.** A port already in use on the host (another login, a host
  service) aborts the launch with a clear message. ops does not pick an ephemeral substitute
  — the tool's redirect URL is fixed, so a different port would silently break the callback.
  Two simultaneous `ops app codex` logins collide on 1455; the second fails (login is a
  one-shot, acceptable).
...
`--forward <port[,port…]>` (repeatable) and `OPS_FORWARD` (a comma-list) add ports for one
...
```
ops app codex --forward 1455
OPS_FORWARD=1455,8080 ops run -- ./dev-server
```
...
allowlist, so `ops app codex` → `codex login` completes end-to-end: codex opens the auth URL
...
- [`ops help run`](../cli/run.md) — the one-shot override flags.
