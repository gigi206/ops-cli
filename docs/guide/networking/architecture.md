...
This page explains how a [filtering egress posture](modes.md) works under the hood, the design called **Model B**: and why it was chosen. It is the *why* behind the
...
[`configuration/gui.md#offscreen`](../configuration/gui.md#offscreen) shows how a small
...
A [`tcp://` L4 rule](rules.md#raw-l4-splice-tcp) opts out of this: the proxy splices the
...
[`[secret]` credential is injected](../secrets/injection.md) into an allowed request, host-side, after the verdict, so the plaintext never enters the cage: and where the
...
- [Secrets architecture](../secrets/README.md): injection and redaction over this
  same proxy.
- [Security model](../concepts/security-model.md) · [Enforcement stack](../concepts/enforcement.md) · [Configuration: `gui`](../configuration/gui.md) (the GUI dummy0 nuance above)
- Design: [threat model and binds](https://github.com/gigi206/ops-cli/blob/ops-v2/docs/bwrap-threat-model-and-binds.md) (the full Model-B-vs-P evidence: the egress section above)
