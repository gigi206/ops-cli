...
[`sbx proc logs`](../cli/proc.md) and [`sbx fs logs`](../cli/fs.md) readers attach
...
See also: [`sbx proc ls`](../cli/proc.md) · [`sbx proc logs`](../cli/proc.md) ·
[`sbx fs logs`](../cli/fs.md) · [`sbx run`](../cli/run.md).
...
- [`sbx run --observe`](../cli/run.md) — enabling observation on a launch
- [`sbx proc`](../cli/proc.md) — `ls`, `logs`, `logs --follow --json`
- [`sbx fs`](../cli/fs.md) — `logs` (the filesystem lens reader)
- [The trust gate](trust.md) — observation is a host-side lens, not a security field
- Design rationale is recorded in this page (process tree + filesystem lens, host-side only, no new attack surface) and in [`bwrap-architecture.md`](https://github.com/gigi206/ops-cli/blob/ops-v2/docs/bwrap-architecture.md).
