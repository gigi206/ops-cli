# `sbx` (bwrap): architecture skeleton

> Blueprint for the new `sbx` (bubblewrap substrate + daemonless nix). Synthesizes
> the threat model + decisions
...
**Disappearing**: `src/build.rs` (legacy image build), the legacy container-runtime
wrapping (~39 K); `clean.rs` + `status.rs` → merged into the **`session/`** module
...
> - **`sbx run`: exec-replace.** Non-interactive: sbx execs bwrap and is
>   replaced by it; the command inherits the real stdio and its exit status
>   becomes sbx's. Spec is `NewSession` (`--new-session`). Do not "simplify"
>   `shell` into this.
> - **`sbx shell`, pty supervisor.** Interactive: sbx stays alive, opens a pty,
...
> **As built, `session/` is the daemonless registry; `sbx session ls` lists it.**
...
> trips. Both launch paths register: `sbx run` (the Mode-B agent path) persists
> its record and is reclaimed by liveness pruning after it execs away; `sbx shell`
...
> stale store generations, and an `sbx session attach <id>` ergonomic.
...
> > capability-bearing more conclusively than the `unshare` stand-in could, bubblewrap cannot nest its namespaces on a cap-stripped one. The stand-in
> (`probe_userns`) is **demoted, not deleted**: it stays the fast gate the launch
> path uses (no subprocess per `sbx run`) and the failure classifier. On a failed
...
> (`sbx trust`/`untrust`/`trust --show`) ahead of any config parsing, because the
> trust marker hashes the **whole file** (the direnv model), so it needs no
> schema: a marker under `$XDG_STATE_HOME/sbx/trusted/` holds a SHA-256 of the
...
> **As built, config layering + gating (`config/`), `sbx config` shows the
> result.** The loader's policy core (`config::resolve`) is **pure** and
> matrix-tested: the global `sbx.toml` is **trusted by location** (safety-gated,
> never marker-gated) and honored in full; the project `.sbx.toml` is layered on
...
> userland sbx owns (`HOME`/`PATH`), because the threat it answers is the same one
...
> launch, so `sbx config` advertises the *effective* binds and cannot drift from what
> the launch binds. Expansion runs *before* the control-plane guard, which sees the
> canonical path, so `~/.local/share/sbx` is still forced read-only like the absolute
> form, no bypass. A bind is **read-only by default**, or **read-write** with the
> table form `{ path = "...", mode = "rw" }` (mapped to bwrap's `--bind`); a
> read-write bind *at or inside* one of sbx's own control-plane roots (the data/engine,
...
> `sbx config` may list it while the launch drops it. One that is an **ancestor**
...
> **As built, the base userland moved into sbx's own store (M3.1), on a rolling
...
> **provisions it into sbx's user-owned store** via `store::provision` (daemonless
...
> sbx binary updates** (an update cannot move tool versions; a nix-free test using
...
> cache to populate the store (the §7.4 "sbx owns its store" tradeoff). The
> configurable channel override, the per-project lock, and `sbx upgrade` are the
...
> channel override plumbs in one place. `sbx config` shows the declared set with
...
> chooses the channel; `sbx config` shows the effective source (project pin / global /
...
> **As built, `sbx upgrade` rolls the channel forward; `doctor`/`config` show the
> locked revision (M3.2d).** Versions never move on an sbx binary update: only an
> explicit `sbx upgrade [all|nix]` does (`all` covers every managed channel; today
...
> > force-re-resolve + report old→new), and `sbx config` (`.locked_revision`, display), so the lock `upgrade` writes is provably the lock a launch reads (no drift). `doctor`
...
>
> Lock writes are **atomic** (temp + `rename`), prompted by the "what if two `sbx
...
> > upgrades racing settle on a last-writer-wins of two *valid* revisions (no `flock`, serialising would only avoid a redundant metadata fetch; the next upgrade reconciles).
>
> **As built, trust composes over the `.sbx.toml` *and* a sibling mise file (M3.3a,
...
> must be the single authority over *both* declarative inputs. `sbx trust` now hashes
> the `.sbx.toml` **and** every sibling mise file together (an unambiguous filename-
...
> loader (`config::read_project`) and in `sbx trust --show` (`trust::state`), so the
> three never diverge. Every input is read through the same safety gate as the
> > `.sbx.toml`: a present-but-unsafe mise file is **unverifiable**, so it fails closed, `sbx trust` refuses to record, and the loader/`--show` report `Untrusted` rather than
> trust a file they cannot read. `sbx config` shows a `mise:` line: the discovered
> file(s) and whether they would be honored (`trusted` / `withheld: <reason>`): without
> running mise, nix, or the network.
>
> Two decisions lock in here. (1) **Anchored on the `.sbx.toml`**: a mise file is hashed
> and honored only beside one (the trust marker is keyed by the `.sbx.toml` path); a
> mise file with no `.sbx.toml` is not honored and `sbx config` surfaces the no-op as a
...
>
> **As built, the mise engine is provisioned via nix and driven from sbx's store
> (M3.3b, glibc-independent scaffolding for the mise front-end).** `sandbox::mise`
> realises the `mise` attribute into sbx's own store (never the host's mise) and runs
...
> live under sbx's store root instead: so the driver runs mise inside a minimal
> bubblewrap that binds sbx's store at `/nix`, the same mechanism the sandbox uses for
> its userland, applied to a tool sbx runs itself. The mount set is empirical (a live
...
> redirected under sbx's data directory (`<data>/mise/`, owner-only), the environment is
...
> proven live by a run that writes solely into sbx's data directory. The provision +
...
>   same way the `.sbx.toml` path already does.
>
...
> realise. Precedence is structural < passthrough < mise `[env]` < `.sbx.toml [env]`.
> Proven live end-to-end: `sbx run` exposes the var only once the project is trusted, and
...
>     default, used by `sbx run`). `PrivateTty` omits it because the pty
>     supervisor (`sbx shell`) establishes the session itself and holds the pty
...
| Command | Effect | Mode |
|---|---|---|
| `sbx shell` | interactive dev shell in the project sandbox; `mise activate` (via `--rcfile`) puts the project's activated tools on PATH | A |
| `sbx run -- <cmd>` | runs a command in the sandbox; the agent's activated mise tools are on PATH via the shims dir | A |
| `sbx mise <args>` | runs mise in the open cage; the agent self-equips `nix:` tools into the project's store. `sbx mise use [-g] nix:<pkg>` installs **and activates** (auto-on-PATH in later launches, no repo mutation with `-g`); a bare `sbx mise install` installs only (reachable via `mise exec`) | B (default) |
| `sbx app <name>` | launches a packaged app (claude/gemini/…); the mode is **declared by the app** | B (default) |
| `sbx install <pkg>` | installs a tool for the project (sbx-mediated provisioning into the shared store) | — |
| `sbx trust` / `sbx untrust` | manages trust (content hash, re-validation) | — |
| `sbx config …` | views/edits the layered config | — |
| `sbx test net <url>` | tests a URL against the resolved egress allowlist and names the rule that allows or denies it (no launch); `sbx test <kind>` is the diagnostic family (filesystem/Landlock later) | — |
| `sbx doctor` | checks prerequisites (**userns**), store health | — |
| `sbx self-update` | updates the binary | — |

## 5. Milestone ordering (the DAG)

| M | Title | Content | Deliverable |
|---|---|---|---|
| **M0** | Prerequisites + store decision | `sbx doctor`: **userns absent → hard-fail with remediation, NEVER a silent fallback** (proot = no security boundary); **store mechanism resolved & de-risked (§7.4): shared flat store, ro-consume / rw-provision, build-sandbox ON outside the cage ✓, concurrent provision-rw vs consume-ro ✓, gcroot+GC ✓, trust-gated provisioning (untrusted ⇒ no input-addressed local builds)** | productized doctor **+ store decision locked (§7.4)** |
| **M1** | Minimal sandbox | `SandboxSpec` + `binds.rs` (zones 0/1/2) + FHS userland + the **free bwrap hardening** (`--clearenv`, all namespaces, `no_new_privs`, `--cap-drop ALL`, `--new-session`, same-uid) + **`session/` (registry, 2nd terminal)**; `sbx shell` isolates the host. Also: **`doctor`**, replace the userns *proxy* probe with a real bwrap smoke run through this argv builder. Seccomp + Landlock-FS land at M4 ([security-stack](bwrap-security-stack.md) §9) | usable shell, Mode A |
| **M2** | Config + trust | global/project layering; content-hash trust gate (direnv); gating of untrusted fields | `.sbx.toml` drives the sandbox **safely** |
| **M3** | Tool provisioning | mise+nix bridge; declarative packages; **sbx-mediated provisioning (store rw) via an inside→outside install channel** (the agent requests, sbx provisions, `/nix` is ro at consumption, so the agent cannot `nix install` itself); pinned nixpkgs | reproducible tools |
| **M4** | Apps + **Mode B** | app definitions; policy engine (A/B × trust → holes); least-privilege secret injection; **the seccomp denylist (incl. `io_uring`/`AF_UNIX`) + Landlock-FS enforcement** ([security-stack](bwrap-security-stack.md) §3–4). ⚠️ **ships the flagship with the confidentiality hole OPEN until M6** (injected API key + open network = possible exfiltration, cf. threat-model §1): mitigate by landing the **credential-injection proxy** (security-stack §6) here, so the agent never holds the key | **`sbx app claude` = the differentiator** |
...
| **M6** | **Network policy / allowlist** | netns layer + per-host filter (block metadata/loopback early, complete the chain with a filtering egress proxy); metadata/localhost blocks → allowlist | **closes the confidentiality hole, LAST** |
...
2. **CLI verb for agents.** `sbx app <x>` with mode declared by the app
   (proposed) vs an explicit `sbx agent <x>` that makes posture B visible.
3. **How sbx embeds nix.** Static nix binary **embedded** in the sbx asset,
   or **downloaded** at bootstrap (base closure from a binary cache /
   cachix)? Impacts the asset size and the first `sbx doctor`.
4. **Store mechanism, RESOLVED: single shared flat store + trust-gated
   ...
   **Decision:** a **single shared flat store**, mounted **read-only at
   consumption** and **read-write only during sbx-mediated provisioning**; the
   ...
   **untrusted** `.sbx.toml` provisions **nothing** (M3.2a): a tool's deliverable is
   ...
   over `sbx trust` (a signed tool is content-verified either way), so it is pure
   ...
   **De-risk checklist: all green** (validated daemonless on a user-owned
   ephemeral store; the failure-in-isolation signature confirmed the build
   sandbox actually engaged):
   ...
   **mise-nix bridge fit (M3).** The repo's bridge (`mise/lib/`) already *is*
   ...
   reroot them under sbx's store; (b) the bridge's `nix build` is where check 1
