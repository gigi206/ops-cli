# `sbx` (bwrap): threat model & bind layout

> Design document that **drives everything else** in the new `sbx` (bubblewrap
> substrate + daemonless nix. The layout of
...
**sbx's announced differentiator = Mode B.** → **Mode B is the default
...
3. **Untrusted project** itself: its `.sbx.toml`, its Makefiles/hooks/devshell
   are **attacker-controlled** the moment you `cd` into a cloned repo.

### Trust boundaries
1. Host ↔ sandbox (the bwrap boundary: the main one).
2. **Trusted** config (`~/.config/sbx/sbx.toml`, written by the user) ↔
   **untrusted** project config (`.sbx.toml` of an arbitrary repo).
3. `sbx` the launcher (trusted, runs on the host) ↔ everything inside the sandbox.
...
- The user deliberately sabotaging their own sandbox (but sbx makes the safe
...
anywhere**. So: **as long as the per-host allowlist (the "later-M6 filtering
proxy" work) does not exist, there is NO confidentiality guarantee in v1.**
...
| Mount | Source | Why |
|---|---|---|
| `/nix` (base) | **sbx's trusted base store** (ro lower) | append-only, secret-free; ro = the agent cannot tamper with installed binaries |
...
| `/etc/ssl/certs/ca-bundle.crt`, `…/ca-certificates.crt` | **sbx's own `cacert`**, ro | TLS trust anchor, hermetic, the cage trusts sbx's bundle, not the host's certs (under a network allowlist the egress proxy's per-session CA overrides it) |
...
```
  trusted BASE store     →  --overlay-src  (ro lower, populated ONLY by sbx on the host side)
...
| **Nested container-management sockets** | **DROPPED**: the socket = **root-equivalent on the host** (launch a container with `/` bind-mounted). Not "gated": **absent**. A filtering proxy-broker is **future work**, not a v1 checkbox. | gated + confirmation |
| **ssh-agent** (`$SSH_AUTH_SOCK`) | **off** by default; opt-in via `[ssh_agent] allow` (trusted/global only), which never binds the host agent's socket: sbx serves the cage a **filtering agent** whose identities answer carries only the named keys, whose signature requests are refused for every other key, and whose message types are an allowlist (no add, no remove, no wipe, no unknown extension). **BUILT.** The residual is destination: a signature names its session, not its host, so an allowed key authenticates anywhere it is trusted for the cage's lifetime, narrow the key itself (a deploy key), or constrain it in your own agent with `ssh-add -h`, which holds because the binding message is forwarded, and `confirm = true` puts a host-desktop prompt in front of every signature the cage asks for (the askpass is resolved from sbx's own environment, never the cage's `[env]`; with no helper the cage gets no agent). Auditable rather than silent: every offer, signature and refusal is recorded in a per-session ring read host-side with `sbx ssh-agent logs`, over a socket never bound into the cage. An `[app.<name>.ssh_agent]` grant scopes a key to one app's cage. | scoped opt-in |
...
- **Env**, `WAYLAND_DISPLAY` and `XDG_RUNTIME_DIR`, fixed by sbx; an untrusted `[env]` could only
  mispoint a client at a nonexistent socket (self-DoS), never redirect the bound socket.
- **Best-effort**, with no compositor socket found, sbx warns and runs without the bind (the app
...
- **Fonts**, a fontless cage renders boxes, so the hole provisions a base font set (DejaVu for the
  ...
  for it by name) host-side into sbx's store, **seeds it into the project store** (the cage reads it through `/nix`),
  and binds a generated, self-contained fontconfig configuration read-only at `/opt/sbx/fonts.conf`,
  ...
  and the app runs without fonts). `FONTCONFIG_FILE` is fixed by sbx; an untrusted `[env]` override
...
`/opt/sbx`, the forwarder on cage loopback). A single-cage e2e holds the teeth together: `wayland-info`
...
Feasibility, recipe, and the protocol enumeration are covered in the **M6/M7
surge** alongside the GUI design decisions.

## 6. The trust gate (security-first): DECIDED (option a)

**Decision (2026-06-14): the trust gate IS the validation.** Doing `sbx trust`
...
- **Untrusted project** (default for any unblessed repo): its `.sbx.toml`
...
- **Trusted project** (`sbx trust`): config honored **in full**, on par
...
> validated content" to stay **true over time**: `sbx trust` records a
...
> `.sbx.toml` that gains `bind ~/.ssh` on the next pull would get it silently.
...
2. **Network: handled at the very end.** Open by default until then. ⇒ **the
   ...
   the full per-host allowlist (the filtering egress proxy) is the **last** step.
