# sbx secrets architecture (design)
...
> rest extends. Read with [`bwrap-security-stack.md`](bwrap-security-stack.md). Background evidence
> behind it: code-level reads of contemporary encapsulated-agent CLIs, plugin-vault
> patterns, namespace-free sandbox launchers, identity-aware reverse proxies,
> edge gateways, native git-credential helpers, AWS SDK / STS flows, and
> first-party agent sandboxes.

## 1. The invariant

> **sbx never places a plaintext secret inside the cage.** The agent receives a
> *capability* (authenticate to an allowed host, sign a challenge, run a declared
> operation), never the secret's bytes.

This sits beside sbx's other hard lines (capability-bearing userns → hard-fail, no
proot fallback; shared store immutable). It is what makes sbx best-in-class on
...
Holding a capability ≠ holding the secret. sbx blocks **extraction/portability**, not
...
- **Resolver (SOURCE)**, *where the value comes from*: `env`, `file`, **SOPS**,
  well-known secret-manager backends, AWS Secrets Manager, third-party vault
  apps, the system keyring… A host-side *fetch*. A typed URI scheme
  (`env://`, `file://`, plus registered per-plugin schemes) tells the
  resolver-plugin registry which backend to consult.
...
the plaintext into sbx's host process; the broker consumes it host-side and exposes
...
host side                                   │ cage (empty netns)
  resolver.fetch(ref)  ── plaintext ──▶ broker ── capability ──▶ agent's tool
   (sops -d, vault, …)   (in sbx mem,      (header inject /        (curl, git, psql…)
...
- A **default signed/curated sbx store** ships built-in resolvers; **third-party
...
- A plugin runs **host-side, sandboxed, never in the cage**. A resolver plugin sees
  ...
  risk the user accepts consciously. The resolver's *own* root credential
  (whatever the plugin requires: a vault token, an age key, an AWS access key)
  lives host-side, never in the cage.
...
|---|---|---|---|---|
| **Broker** (HTTP-header / ssh-agent / DB-proxy / mTLS) | **sbx** (the protocol binds the secret to its destination) | **structural**, agent never touches plaintext | yes | a *reflecting/cooperating allowed upstream* (irreducible) |
| **Template** (MCP): `op.run("db-query", {sql})` | **sbx** (fixed command), agent supplies **bounded data params** | structural, per-template vetted | yes | a too-general template; an escape hatch in the templated tool |
...
The line through the table: **safe ⟺ sbx controls the program/sink that touches the
...
1. **Transparent broker / protocol proxy** *(best)*: the secret stays host-side, it is
   **command-agnostic** (any tool works unchanged), leak-safe, and needs **no agent
   ...
   header, the ssh-agent socket, a DB auth packet, a TLS client cert). This: not MCP: is the right way to widen coverage beyond HTTP/SSH.
...
Beyond HTTP/SSH, sbx cannot transparently proxy everything, so the lower tiers are what
make sbx **general** rather than "works only for brokered protocols". They are offered,
but fenced:
...
3. **Egress allowlist MANDATORY** for any exposed/command tier: refuse to combine an
   ...
   interpreter bypasses it), so the egress allowlist + empty netns are what is left, keep it mandatory and tight. **They are not a net.** An exposed secret reaches the
...
an injection target. Host-scoped by default, path on opt-in (the same
conservative default as `git-credential`'s `useHttpPath=false`). One
canonicalizer/matcher across allow / deny / inject.
...
- **`http-header` injection requires a filtering `[network]` mode (`deny`/`allow`/`ask`).** Injection happens
  *inside the MITM proxy*, which only exists under the allowlist posture (`egress::start`
  ...
  `sbx config` surfaces it as a warning (a forgotten `allow` entry is not a silent "why
  is my auth missing").
...
- **Basic input format:** the resolved `from` source (or terse `key`) holds the
  `user:pass` pair; sbx base64-encodes it (the agent never pre-encodes).
...
2. **Launch (host-side, before the cage):** sbx calls the SOPS resolver plugin
   (host-side subprocess) → it uses the host-side age/KMS key to decrypt
   `secrets.enc.yaml` and returns `github.token`'s plaintext over a pipe. sbx
   configures the MITM proxy: "for `api.github.com`, set `Authorization: Bearer
   <token>`". The token is in sbx's host process: not the cage env, not a cage file.
3. **Agent runs:** `curl https://api.github.com/user` (no token anywhere) → in-cage
   ...
   key in the cage).
4. **Teardown:** sbx zeroizes the plaintext; proxy, CA, and socket are torn down.

The "consumption" is the HTTPS call the agent makes anyway; sbx brokers it on the wire.
No MCP, no agent cooperation. MCP is only for the lower tiers (operations sbx cannot
transparently proxy).
...
- **A `network = true` resolver gets unrestricted host egress.** A resolver plugin runs
  **host-side** (outside the agent's cage), so a manifest that declares `network = true`
  ...
  network and is **not** behind the cage's Model-B egress allowlist. Accepted because
  resolvers are in the TCB
...
The *primitives* are all standard (command allowlists ≈ sudoers; TOFU approval ≈
first-party agent CLIs; egress allowlist ≈ dev-container firewalls; broker-side
credential injection ≈ MITM egress proxies). sbx's lead is the **integration**: per-secret broker +
...
1. **6.3a, DONE**, `kind = "http-header"`, built-in `env://` / `file://` resolvers, the
   ...
   fail-closed on a missing/empty source; per-request re-match; secret hygiene, never logged, redacted in `Debug`). **The foundation; proves the wire-injection
...
4. **Resolver-plugin contract, DONE**, instead of baking one backend
   in-tree, sbx ships a **typed resolver-plugin registry**: a plugin declares a `scheme`,
   sbx runs it host-side under bwrap, and a `scheme://locator` `from` ref routes to it.
...
5. **Plugin store, DONE**, the resolver contract generalised into a **remote signed
   store**: `sbx plugins store add/update/info/list/rm` (git-fetched catalogue,
...
7. **ssh-agent forward, DONE** (sign-oracle, key never in the cage): `[ssh_agent] allow`
   names keys by fingerprint or comment; sbx serves the cage a filtering agent socket whose
   ...
   prompt in front of every signature the cage asks for (askpass resolved from sbx's own
