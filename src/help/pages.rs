//! The page table itself: one [`Page`] per command and subcommand, and nothing else.
//!
//! Flat, and one file, on purpose. The single source of truth the module header claims is exactly
//! this — one slice with one definition site — and sharding it per namespace would buy localized
//! diffs at the price of the one failure this shape cannot have: a group of pages nobody
//! aggregates, missing from the surface in silence. The table is read by grepping for a `path:`
//! row rather than from top to bottom, which is why it sits apart from the engine rendering it.
