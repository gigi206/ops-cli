...
/// The read-budget adapter, from the module that owns it.
///
/// Re-exported rather than imported at each use because both planes here read a head through it;
/// the broker's frame reader takes the same adapter from the same place, so the rule about what a
/// socket timeout does not bound is written once and every reader is held to it the same way.
pub(super) use crate::sandbox::deadline::Deadlined;
