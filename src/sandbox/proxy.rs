...
pub(crate) struct ProxyCtx {
    ...
    server_config_h2: Arc<ServerConfig>,
...
mod h2mitm {
    ...
    async fn stream(
        ...
        let deciding: Option<Rule> =
            match policy.explain(connect_host, port, &path, method.as_str()) {
                Decision::AllowedBy(rule) => Some(rule.clone()),
                Decision::AllowedDefault => None,
                Decision::DeniedBy(_) => {
                    ctx.outcome(
    ...
    async fn relay(
        ...
        let tcp =
            match tokio::time::timeout(ctx.timeout, tokio::net::TcpStream::connect((ip, port)))
                .await
            {
                Ok(Ok(t)) => t,
                _ => {
        ...
        let upstream_tls =
            match tokio::time::timeout(ctx.timeout, connector.connect(name, tcp)).await {
                Ok(Ok(t)) => t,
        ...
        for (name, value) in parts.headers.iter() {
            let n = name.as_str();
            if forbidden_request_header(n)
                || injected.iter().any(|(h, _)| h.eq_ignore_ascii_case(n))
            {
                continue;
            }
...
fn flush_tls<D: rustls::SideData>(
...
fn read_plaintext<D: rustls::SideData>(
    conn: &mut rustls::ConnectionCommon<D>,
    sock: &mut impl Read,
    buf: &mut [u8],
...
mod tests {
    ...
    fn a_burst_larger_than_the_read_buffer_then_idle_is_fully_delivered() {
        ...
        let got = through_proxy_ws_read_exact(ctx, proxy_ca_der, "upstream.test", addr.port(), N)
            .unwrap();
        assert!(
            got == bulk_payload(N),
