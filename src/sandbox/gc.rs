...
pub(crate) struct PrunedCache {
    /// The entry's name directly under the home's cache directory (`mise`, `uv`, `npm`), for the
    /// report — what the user reads to recognise which tool has to refill.
    pub(crate) name: String,
    /// On-disk size of the tree that was (or would be) freed.
    pub(crate) bytes: u64,
}

/// Remove the caches an app's home carries — `sbx app prune --caches`. The directory is
/// `<home>/.cache`, which the XDG base directory specification defines as non-essential data whose
/// loss costs a refetch and nothing else; that contract is what makes emptying it sound without a
/// per-tool allowlist, which no set of names could keep current anyway. Each entry directly under it
/// is removed whole and reported by name, so what has to refill is legible rather than a single
/// opaque total. With `apply = false` nothing is removed — the return is the preview of what would
/// go. An app's login and session state lives in `.config` and `.local/share`, neither of which this
/// touches.
pub(crate) fn prune_app_caches(home: &Path, apply: bool) -> Vec<PrunedCache> {
    // Resolved and confined before anything is enumerated or removed, for the reason
    // [`prune_app_tools`] states at length: `home` is the cage's own `$HOME`, so `.cache` is an
    // ordinary directory untrusted in-cage code owns and can replace with a symlink out of the home.
    // [`force_remove_dir_all`] lstats only the root it is handed — here an entry *inside* `.cache` —
    // so a link standing where `.cache` itself does would go unexamined, and `read_dir` would
    // enumerate somebody else's directory as this app's caches for a recursive delete.
    let Some(cache) = contained_in(home, &home.join(".cache")) else {
        return Vec::new();
    };
    ...
    for (name, size) in names.into_iter().zip(sizes) {
        ...
        if apply {
            let removed = match std::fs::symlink_metadata(&entry) {
                Ok(meta) if meta.is_dir() => force_remove_dir_all(&entry).is_ok(),
                Ok(_) => std::fs::remove_file(&entry).is_ok(),
                Err(_) => false,
            };
            if !removed {
                continue;
...
/// One entry `sbx app prune --drop` or `--reset` would take (or took) from a home.
...
/// Remove the entries `rels` names from `home` — `sbx app prune --drop`.
///
/// Nothing about a name is interpreted. What a directory holds is the reader's to judge, which is
/// the same reason `sbx app show` lists a home rather than sorting it: an app's own data and a
/// package manager's downloads sit side by side under one parent and are told apart by nothing a
...
/// Remove everything a home holds, leaving the home itself — `sbx app prune --reset`.
///
...
///
/// One walk rather than one per entry, so a file linked into two of them is counted once — the
/// reason [`prune_app_caches`] sizes its entries the same way.
...
mod tests {
    ...
    fn home_with(root: &Path, entries: &[(&str, usize)]) -> PathBuf {
        let home = root.join("home");
        for (rel, bytes) in entries {
            let dir = home.join(rel);
            std::fs::create_dir_all(&dir).unwrap();
            std::fs::write(dir.join("f"), vec![b'x'; *bytes]).unwrap();
        }
        home
    }

    #[test]
    fn drop_takes_what_it_is_named_and_leaves_the_rest() {
        let tmp = TmpDir::new();
        let home = home_with(
            tmp.path(),
            &[(".rustup", 4096), (".npm", 2048), (".config", 128)],
        );

        let taken = drop_home_entries(&home, &[".rustup".to_string(), ".npm".to_string()], true);

        let named: Vec<&str> = taken.iter().map(|e| e.rel.as_str()).collect();
        assert_eq!(named, [".npm", ".rustup"]);
        assert!(!home.join(".rustup").exists());
        assert!(!home.join(".npm").exists());
        assert!(
            home.join(".config/f").exists(),
            "an entry nobody named was taken"
        );
    }

    #[test]
    fn drop_reaches_a_nested_entry_by_its_relative_path() {
        // The name `sbx app show` prints once the view has descended, which is the only name a
        // reader has for what sits under a passthrough directory.
        let tmp = TmpDir::new();
        let home = home_with(
            tmp.path(),
            &[(".local/share/pnpm", 4096), (".local/share/ours", 2048)],
        );

        let taken = drop_home_entries(&home, &[".local/share/pnpm".to_string()], true);

        assert_eq!(
            taken.len(),
            1,
            "expected one entry, got {taken:?}",
            taken = taken.len()
        );
        assert!(!home.join(".local/share/pnpm").exists());
        assert!(home.join(".local/share/ours/f").exists());
    }

    #[test]
    fn drop_refuses_a_name_that_resolves_out_of_the_home() {
        let tmp = TmpDir::new();
        let outside = tmp.path().join("outside");
        ...
        let taken = drop_home_entries(
            &home,
            &["../outside".to_string(), ".escape".to_string()],
            true,
        );
    ...
    #[test]
    fn drop_counts_an_entry_named_inside_another_only_once() {
        // Both names reach the same bytes; sizing them as two parts of one walk would report the
        // inner one as empty, and the outer removes it regardless.
        let tmp = TmpDir::new();
        let home = home_with(tmp.path(), &[(".local/share/pnpm", 4096)]);

        let taken = drop_home_entries(
            &home,
            &[".local/share/pnpm".to_string(), ".local".to_string()],
            false,
        );

        let named: Vec<&str> = taken.iter().map(|e| e.rel.as_str()).collect();
        assert_eq!(named, [".local"]);
        assert!(taken[0].bytes >= 4096, "the outer entry lost its size");
    }

    #[test]
    fn drop_previews_without_removing_anything() {
        let tmp = TmpDir::new();
        let home = home_with(tmp.path(), &[(".rustup", 4096)]);

        let taken = drop_home_entries(&home, &[".rustup".to_string()], false);

        assert_eq!(taken.len(), 1);
        assert!(home.join(".rustup/f").exists(), "a preview removed a tree");
    }

    #[test]
    fn a_reset_empties_the_home_and_keeps_the_home() {
        let tmp = TmpDir::new();
        let home = home_with(
            tmp.path(),
            &[(".rustup", 4096), (".config", 128), (".local/share", 512)],
        );
        std::fs::write(home.join(".bash_history"), b"one line").unwrap();

        let taken = reset_home(&home, true);

        let mut named: Vec<&str> = taken.iter().map(|e| e.rel.as_str()).collect();
        named.sort_unstable();
        assert_eq!(named, [".bash_history", ".config", ".local", ".rustup"]);
        assert!(home.is_dir(), "the home itself must survive its reset");
        assert_eq!(
            std::fs::read_dir(&home).unwrap().count(),
            0,
            "the home still holds something after a reset"
        );
    }

    #[test]
    fn a_reset_previews_without_removing_anything() {
        let tmp = TmpDir::new();
        let home = home_with(tmp.path(), &[(".config", 128)]);

        let taken = reset_home(&home, false);
    ...
    /// The cache directory is in the cage's own `$HOME`, so in-cage code can replace it with a link
    /// out of the home. `force_remove_dir_all` lstats only the root it is handed, and the roots this
    /// verb hands it are entries *inside* the cache directory: a link standing where the cache
    /// directory itself does would go unexamined, and `read_dir` would enumerate somebody else's
    /// directory as this app's caches for a recursive delete.
    #[test]
    fn prune_caches_refuses_a_cache_dir_pointed_out_of_the_home() {
        let tmp = TmpDir::new();
        let home = tmp.path().join("home");
        let outside = tmp.path().join("outside");
        std::fs::create_dir_all(outside.join("Documents/notes")).unwrap();
        ...
        let pruned = prune_app_caches(&home, true);

        assert!(
            pruned.is_empty(),
            "a cache dir resolving outside the home must yield nothing prunable, got {:?}",
            pruned.iter().map(|p| &p.name).collect::<Vec<_>>()
        );
        assert!(
            outside.join("Documents/notes/keep.txt").exists(),
            "the host directory the link pointed at was deleted"
        ...
        assert!(
            std::fs::symlink_metadata(home.join(".cache"))
                .unwrap()
                .file_type()
                .is_symlink(),
            "the planted link itself must be left alone, not removed"
    ...
    /// The ordinary case: every entry under the cache directory goes, whether it is a tree or a
    /// plain file, and the two directories an app's login and session state live in are untouched.
    /// Without this the guard above could be satisfied by a function that always refuses.
    #[test]
    fn prune_caches_removes_every_entry_and_leaves_config_and_data_alone() {
        let tmp = TmpDir::new();
        let home = tmp.path().join("home");
        let cache = home.join(".cache");
        for tool in ["mise", "uv"] {
            std::fs::create_dir_all(cache.join(tool).join("downloads")).unwrap();
            std::fs::write(cache.join(tool).join("downloads").join("blob"), b"xxxx").unwrap();
        }
        std::fs::write(cache.join("loose.log"), b"x").unwrap();
        std::fs::create_dir_all(home.join(".config/app")).unwrap();
        std::fs::write(home.join(".config/app/auth.json"), b"token").unwrap();
        std::fs::create_dir_all(home.join(".local/share/app")).unwrap();
        std::fs::write(home.join(".local/share/app/state"), b"kept").unwrap();

        let pruned = prune_app_caches(&home, true);

        assert_eq!(
            pruned.iter().map(|p| p.name.clone()).collect::<Vec<_>>(),
            vec![
                "loose.log".to_string(),
                "mise".to_string(),
                "uv".to_string()
            ],
            "every entry is reported, in a stable order"
        );
        assert!(!cache.join("mise").exists(), "a cache tree goes");
        assert!(!cache.join("loose.log").exists(), "a loose file goes too");
        assert!(
            home.join(".config/app/auth.json").exists(),
            "login state in .config must survive a cache prune"
        );
        assert!(
            home.join(".local/share/app/state").exists(),
            "state in .local/share must survive a cache prune"
        );
    }

    /// A preview reports what would go and removes none of it, so the entries are still there
    /// afterwards. The verb previews by default, and a preview that deleted would make the default
    /// the destructive one.
    #[test]
    fn prune_caches_previews_without_removing_anything() {
        let tmp = TmpDir::new();
        let home = tmp.path().join("home");
        let cache = home.join(".cache");
        std::fs::create_dir_all(cache.join("mise")).unwrap();
        std::fs::write(cache.join("mise").join("blob"), b"xxxx").unwrap();
        ...
        assert_eq!(
            pruned.iter().map(|p| p.name.clone()).collect::<Vec<_>>(),
            vec!["mise".to_string()],
            "the preview names what would go"
        );
        assert!(
    ...
    /// An entry whose removal fails is not reported: the verb prints a freed total, and naming a
    /// tree as pruned that is still on the disk tells the user bytes came back that did not. A
    /// recursive removal can fail having already emptied what it walked, so the entry surviving —
    /// not its contents — is what says the removal did not go through.
    #[test]
    fn prune_caches_does_not_report_an_entry_it_could_not_remove() {
        use std::os::unix::fs::PermissionsExt;
        let tmp = TmpDir::new();
        let home = tmp.path().join("home");
        let cache = home.join(".cache");
        std::fs::create_dir_all(cache.join("mise")).unwrap();
        std::fs::write(cache.join("mise").join("blob"), b"xxxx").unwrap();
        // Unlinking an entry needs write on the directory holding it, not on the entry itself.
        std::fs::set_permissions(&cache, std::fs::Permissions::from_mode(0o500)).unwrap();

        let pruned = prune_app_caches(&home, true);

        // Restored before any assertion, so a failing assertion still leaves a removable fixture.
        std::fs::set_permissions(&cache, std::fs::Permissions::from_mode(0o700)).unwrap();
        assert!(
            pruned.is_empty(),
            "an entry that could not be removed must not be reported as freed, got {:?}",
            pruned.iter().map(|p| &p.name).collect::<Vec<_>>()
        );
        assert!(
            cache.join("mise").exists(),
            "the fixture only tests anything if the removal really failed"
        );
    }
