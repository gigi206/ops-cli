...
//!
//! Under an allowlist sbx runs a TLS-terminating proxy with a per-session CA and injects that
...
//! so a graphical app rejects sbx's CA (`ERR_CERT_AUTHORITY_INVALID`) and its UI cannot load.
//!
//! This is conceptually part of the Wayland GUI hole (like fonts): when a GUI cage also runs a
//! filtering posture, sbx provisions `certutil` and prepends a step that imports the bound CA
//! into the cage's NSS db before the app runs. No new trust is granted — the cage already trusts
//! sbx's MITM CA via the env vars; this only extends the *same* trust to the store Chromium
...
/// The NSS nickname for sbx's imported CA, and the prefix the wrap purges. Every per-session CA
/// shares the **same subject DN** (`CN=sbx egress proxy CA`) with a different key, so if several
...
/// HTTPS with `ERR_CERT_AUTHORITY_INVALID`. So the wrap **purges every `sbx-mitm*` entry before
...
const CA_NICKNAME: &str = "sbx-mitm";
...
/// Provision `certutil` into sbx's store against the pinned `nixpkgs`, sharing the revision-keyed
...
/// interpolated into the script — the only interpolated values are the sbx-controlled certutil
...
pub(crate) fn wrap(
    ...
    //
    // Purge every prior `sbx-mitm*` CA before re-adding the current one under a fixed nickname, so
    ...
    // single-instance GUI apps (a second `sbx app <name>` hands off to the running one rather than
...
mod tests {
    ...
    fn wrap_imports_the_ca_then_execs_the_command_positionally() {
        ...
        assert!(script.contains("-A -n sbx-mitm -t 'C,,' -i '/opt/sbx/egress-ca.pem'"));
