...
/// to itself; the hash is fetched via `nix store prefetch-file`, which follows redirects and adds the
/// file to sbx's store. `fresh` marks an `sbx upgrade` re-resolve: with no source query of its own,
