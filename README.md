...
| `config` | Dumps the effective config (all `OPS_*` scalars + arrays) with origin (env/config/default). With a subcommand, performs managed edits to `ops.conf`: `config set/get/unset KEY [VALUE]` for scalars, `config alias add/remove NAME [argv...]` for `OPS_ALIASES[]`, `config secret add/list/remove NAME` for tokens (chmod 600 enforced; values never echoed by `list`). All edits are atomic (mktemp + mv) and idempotent. |
...
**557 tests across 42 files.** The "coverage" column above is an eyeballed estimate based on which documented subcommands and flags are exercised; no coverage tool is run in CI (see `mise run coverage` for an opt-in local report, with caveats).
