...
function M.choose_store_path_with_bin(outputs)
  ...
  for _, path in ipairs(outputs) do
    ...
    local q = shell.shquote(bin_path)
    local has_bin = shell.exec("test -d " .. q .. " && echo yes || echo no"):match("yes") ~= nil
    local bin_count = 0

    if has_bin then
      bin_count = tonumber(shell.exec("ls -1 " .. q .. " 2>/dev/null | wc -l")) or 0
    end
...
function M.verify_build(chosen_path, tool)
  ...
  local qp = shell.shquote(chosen_path)
  local exists = shell.exec("test -e " .. qp .. " && echo yes || echo no"):match("yes")
  if not exists then
  ...
  local qb = shell.shquote(bin_path)
  local has_bin_dir = shell.exec("test -d " .. qb .. " && echo yes || echo no"):match("yes")
  if has_bin_dir then
