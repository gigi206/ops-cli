...
by [`sbx projects rm`](../cli/projects).

See also: [`sbx gc`](../cli/gc) · [`sbx store`](../cli/store) · [`sbx projects`](../cli/projects) · [Provisioning](../concepts/provisioning) · [Directory layout](../concepts/directory-layout).
...
immediately. See [Deduplication](../cli/gc#deduplication).
...
[`sbx app prune`](../cli/app); an inline `[flakes]` build is reclaimed once its name
leaves the config.
...
[`sbx session stop`](../cli/session), a detached session killed later) and the cleanup
...
the data [`sbx net stats`](../cli/net) aggregates (`sbx net stats --reset` is their purge).
