...
[`[task.<name>]`](../configuration/task). `sbx tasks` is an alias.
...
See also: [`[task]`](../configuration/task) · [`sbx secret list`](secret) ·
[`sbx session`](session).
...
each [bundle](../configuration/bundles) that profile names in `use`. The name alone does not say
...
[`sbx session ls`](session) already uses in its `KIND` column. A bundle keeps its own name even
...
This does **not** duplicate [`sbx session ls`](session), which names the app and the project but
...
value outside its bound, a variable not in `env_allow`, or an [output directory](../configuration/task)
...
[`[task.defaults]`](../configuration/task) it inherits, and those can live in different files, `declared in project` with a ceiling from your global `sbx.toml` is an ordinary situation, and being
