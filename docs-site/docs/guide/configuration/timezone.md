...
to know where its user is). A `timezone` written on an app, whether under an `[app.<name>]` or at
the top level of a profile file, is reported at launch rather than silently ignored, and the report
