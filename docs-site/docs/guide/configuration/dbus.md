...
See also: [`gui`](gui) · [`gpu`](gpu) · [Enforcement stack](../concepts/enforcement) · [The trust gate](../concepts/trust) · [`[app.<name>]`](apps).
...
- **File chooser**: the dialog is **rendered in-cage** by the GTK backend, so by construction it can
  ...
  [`binds`](binds) mounts), because the backend runs in the cage's mount namespace. It is the
...
for that socket (Mutter safe; wlroots exposes screen-capture/input-injection: see [`gui`](gui))
