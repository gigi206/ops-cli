...
See also: [`gui`](gui) · [`gpu`](gpu) · [`dbus`](dbus) · [Enforcement stack](../concepts/enforcement) · [The trust gate](../concepts/trust) · [`[app.<name>]`](apps).
...
The PulseAudio bus grants a connected client the microphone **and** every `.monitor` source, that is, the ability to record all system audio output (other apps, a meeting), not only the
