...
the aggregator that re-exposes its model catalogue behind OpenAI-, Anthropic-, and Gemini-compatible endpoints. The mechanics, posture, and scoping are in [Secrets](../); this page only adds what is specific to TokenRouter.
...
| Body | Meaning |
|---|---|
| `Token not provided` | nothing was injected: check the filtering posture and that the allowlist reaches the host (see [Secrets](../)) |
