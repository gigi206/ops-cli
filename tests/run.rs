...
/// on disk), and it is reclaimed by removing that tree.
...
fn a_gui_isolated_cage_gets_a_dummy_interface_a_non_gui_one_does_not() {
    ...
    if !probe.status.success() {
        skip_incapable!("skipping netns-dummy e2e: host cannot sandbox");
        return;
    }
    let t = sbx_in(
        gui.path(),
    ...
    if !ifaces.contains("dummy0") {
        skip_incapable!(
            "skipping netns-dummy e2e: dummy interface unavailable on this host:\n{ifaces}"
        );
        return;
    }
...
fn a_gui_dummy_interface_opens_no_egress_the_allowlist_still_filters() {
    ...
    if !String::from_utf8_lossy(&ifaces.stdout).contains("dummy0") {
        skip_incapable!("skipping dummy-egress e2e: dummy interface unavailable on this host");
        return;
    }
...
fn sbx_gc_keeps_a_current_flake_build_and_reclaims_a_rolled_away_one() {
    ...
    {
        skip_incapable!(
            "skipping sbx gc e2e: the flake build did not complete host-side: {built_log}"
        );
        return;
    }
...
fn a_trusted_devices_grant_binds_a_host_device_into_the_cage() {
    ...
    let Some(device) = ["/dev/net/tun", "/dev/fuse", "/dev/kvm", "/dev/dri"]
        ...
        skip_incapable!("skipping devices e2e: no candidate host device present");
        return;
    };
...
fn a_trusted_audio_posture_binds_the_pulseaudio_socket_into_the_cage() {
    ...
    match &host_socket {
        ...
        _ => {
            skip_incapable!(
                "skipping audio e2e: no PulseAudio socket at $XDG_RUNTIME_DIR/pulse/native"
            );
            return;
        }
    }
...
fn a_typed_one_shot_security_override_reaches_the_cage() {
    let Some(device) = ["/dev/net/tun", "/dev/fuse", "/dev/kvm", "/dev/dri"]
        ...
        skip_incapable!("skipping one-shot override e2e: no candidate host device present");
        return;
    };
...
fn sbx_attach_joins_the_live_cage_with_the_confinement_reapplied() {
    ...
    if !cage_userns_ready(session_pid) {
        ...
        let _ = agent.wait();
        skip_incapable!(
            "skipping attach e2e: the background cage never came up (userns not created)"
        );
        return;
    }
