#!/usr/bin/env bats
# config set/get/unset on bracketed keys (OPS_AGENT_FLAGS[claude],
...
@test "config set OPS_AGENT_FLAGS[claude] bootstraps declare -A on fresh file" {
    [ ! -f "$OPS_CONF" ]
    run "$(ops_sh)" config set 'OPS_AGENT_FLAGS[claude]' "--claude-volume --install"
    assert_success
    [ -f "$OPS_CONF" ]
    grep -qE '^[[:space:]]*declare[[:space:]]+-A[[:space:]]+OPS_AGENT_FLAGS' "$OPS_CONF"
    grep -qF 'OPS_AGENT_FLAGS[claude]="--claude-volume --install"' "$OPS_CONF"
}

@test "config set OPS_AGENT_FLAGS[claude] does NOT duplicate declare -A on second add" {
    "$(ops_sh)" config set 'OPS_AGENT_FLAGS[claude]' "--claude-volume"
    run "$(ops_sh)" config set 'OPS_AGENT_FLAGS[gemini]' "--gemini-volume"
    assert_success
    [ "$(grep -c 'declare -A OPS_AGENT_FLAGS' "$OPS_CONF")" = "1" ]
    grep -qF 'OPS_AGENT_FLAGS[claude]="--claude-volume"' "$OPS_CONF"
    grep -qF 'OPS_AGENT_FLAGS[gemini]="--gemini-volume"' "$OPS_CONF"
}
...
@test "config set bracketed is idempotent — same value no-op" {
    ...
    sleep 0.1
    run "$(ops_sh)" config set 'OPS_AGENT_FLAGS[claude]' "--claude-volume"
    assert_success
    local hash2
...
@test "config set bracketed updates an existing entry in place" {
    "$(ops_sh)" config set 'OPS_AGENT_FLAGS[claude]' "--claude-volume"
    run "$(ops_sh)" config set 'OPS_AGENT_FLAGS[claude]' "--claude-volume --install --no-rm"
    assert_success
    grep -qF 'OPS_AGENT_FLAGS[claude]="--claude-volume --install --no-rm"' "$OPS_CONF"
    [ "$(grep -c '^OPS_AGENT_FLAGS\[claude\]=' "$OPS_CONF")" = "1" ]
}
...
@test "config set rejects bare array name (no bracket)" {
    run "$(ops_sh)" config set OPS_AGENT_FLAGS "anything"
    assert_failure
    assert_output_contains "is an array"
    assert_output_contains "config set OPS_AGENT_FLAGS"
}
...
@test "config get OPS_AGENT_FLAGS[claude] reads the value" {
    "$(ops_sh)" config set 'OPS_AGENT_FLAGS[claude]' "--claude-volume --install"
    run "$(ops_sh)" config get 'OPS_AGENT_FLAGS[claude]'
    assert_success
    [ "$output" = "--claude-volume --install" ]
}

@test "config get bracketed returns 1 + diagnostic when entry absent" {
    run "$(ops_sh)" config get 'OPS_AGENT_FLAGS[nonexistent]'
    assert_failure
...
@test "config get bracketed reads the right entry when others coexist" {
    "$(ops_sh)" config set 'OPS_AGENT_FLAGS[claude]' "--claude-volume"
    "$(ops_sh)" config set 'OPS_AGENT_FLAGS[gemini]' "--gemini-volume"
    run "$(ops_sh)" config get 'OPS_AGENT_FLAGS[gemini]'
    assert_success
...
@test "config unset OPS_AGENT_FLAGS[claude] removes only that entry" {
    "$(ops_sh)" config set 'OPS_AGENT_FLAGS[claude]' "--claude-volume"
    "$(ops_sh)" config set 'OPS_AGENT_FLAGS[gemini]' "--gemini-volume"
    run "$(ops_sh)" config unset 'OPS_AGENT_FLAGS[claude]'
    assert_success
    ! grep -qF 'OPS_AGENT_FLAGS[claude]=' "$OPS_CONF"
    grep -qF 'OPS_AGENT_FLAGS[gemini]="--gemini-volume"' "$OPS_CONF"
}

@test "config unset bracketed is idempotent" {
    "$(ops_sh)" config set 'OPS_AGENT_FLAGS[claude]' "--claude-volume"
    "$(ops_sh)" config unset 'OPS_AGENT_FLAGS[claude]'
    run "$(ops_sh)" config unset 'OPS_AGENT_FLAGS[claude]'
    assert_success
}
...
@test "OPS_AGENT_FLAGS[claude] set via config set drives --claude isolation at run time" {
    # Wire the per-agent flag, then verify ops run --claude --dry-run picks it up.
    "$(ops_sh)" config set 'OPS_AGENT_FLAGS[claude]' "--claude-volume"
...
@test "config alias add still works after refactor (regression guard)" {
    ...
    grep -qF 'OPS_ALIASES[cc]="run --claude"' "$OPS_CONF"
}

@test "config alias add coexists with config set OPS_AGENT_FLAGS[…]" {
